...
export function validateMove(context) {
    ...
    const maxPlayable = Math.max(context.playerHand.length, context.playerTableVisible.length, context.playerTableBlind.length);
 ...
 */
function findCardsInZone(cardIds, zone) {
    const found = [];
 ...
 */
function validatePileLegality(cardsToPlay, currentPile, activeConstraints) {
 ...
 */
export function checkConsecutiveBomb(pile) {
    if (pile.length === 0) {
...
export function evaluateConstraint(cardsPlayed, oldPile, newPile) {
 ...
 */
export function evaluatePickup(playerHand, playerTableVisible, playerTableBlind, currentPile, activeConstraints) {
    // No pickup if can play
    if (playerHand.length > 0) {
        // Check if any card beats pile
        const canBeat = playerHand.some(c => c.isWildcard || c.value >= (currentPile[currentPile.length - 1]?.value || 0));
        if (canBeat)
            return 'none';
        // Check 7 constraint
        if (activeConstraints.sevenOrUnder) {
...
//# sourceMappingURL=MoveValidatorService.js.map
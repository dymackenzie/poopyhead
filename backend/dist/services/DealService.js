 ...
 */
export function calculateHandSize(playerCount) {
    if (playerCount % 5 === 0) {
        return 4;
    }
    return 5;
 ...
 */
export function dealGame(deck, playerCount) {
    ...
    if (deck.length < totalCardsNeeded) {
        throw new Error(`Insufficient cards in deck: need ${totalCardsNeeded}, have ${deck.length}`);
    }
    const playerHands = [];
    const playerTableBlind = [];
    const playerTableVisible = [];
    let deckCopy = [...deck];
...
export function determineFirstPlayer(players) {
    ...
    if (maxPoopyheadCount > 0) {
        ...
        if (poopyheadCandidates.length === 1) {
            ...
            bestPlayer = poopyheadCandidates.reduce((best, curr) => curr.fourCountInHand > best.fourCountInHand ? curr : best);
            reason = 'tied for most Poopyheads, won tiebreak with most 4s';
        }
    }
    else {
        // All have 0 Poopyheads (new game); use most 4s
        bestPlayer = players.reduce((best, curr) => curr.fourCountInHand > best.fourCountInHand ? curr : best);
    }
    return {
 ...
 */
export function countFoursInHand(hand) {
